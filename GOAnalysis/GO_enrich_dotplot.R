...
Y05_BP_plot_table <- cbind("Genotype" = rep('Y05', nrow(Y05_BP_plot_table)), Y05_BP_plot_table)
Y023_BP_plot_table <- cbind("Genotype" = rep('Y023', nrow(Y023_BP_plot_table)), Y023_BP_plot_table)
E05_BP_plot_table <- cbind("Genotype" = rep('E05', nrow(E05_BP_plot_table)), E05_BP_plot_table)
R2_BP_plot_table <- cbind("Genotype" = rep('R2', nrow(R2_BP_plot_table)), R2_BP_plot_table)
PA_BP_plot_table <- cbind("Genotype" = rep('PA', nrow(PA_BP_plot_table)), PA_BP_plot_table)
Sierra_BP_plot_table <- cbind("Genotype" = rep('Sierra', nrow(Sierra_BP_plot_table)), Sierra_BP_plot_table)
...
all_genotype_BP_plot_table <- rbind(Y05_BP_plot_table, Y023_BP_plot_table, E05_BP_plot_table, R2_BP_plot_table, PA_BP_plot_table, 
    ...
    #Make plot
x_axis_BP_order <- factor(all_genotype_BP_plot_table$Genotype, levels = c('Y05', 'Y023', 'E05', 'R2', 'PA', 'Sierra'))

p_BP <- ggplot(data = all_genotype_BP_plot_table, aes(x = x_axis_BP_order, y = Term, size = Significant, color = p_adjusted))
p_BP <- p_BP + geom_point() + scale_color_gradientn(colors = terrain.colors(10), limits=c(0, 0.05)) 
p_BP <- p_BP + xlab('Genotype') + ylab('GO Term') + ggtitle('BP') +theme(plot.title = element_text(hjust = 0.5))
p_BP <- p_BP + labs(color = 'Adjusted p-value', size = 'Gene Rank')
...
Sierra_MF_plot_table <- Sierra_MF_GO_terms[which(Sierra_MF_GO_terms$GO.ID %in% list_all_MF_GO_included), ]
    ...
    #Add a column labeling the genotype to each 
Y05_MF_plot_table <- cbind("Genotype" = rep('Y05', nrow(Y05_MF_plot_table)), Y05_MF_plot_table)
Y023_MF_plot_table <- cbind("Genotype" = rep('Y023', nrow(Y023_MF_plot_table)), Y023_MF_plot_table)
E05_MF_plot_table <- cbind("Genotype" = rep('E05', nrow(E05_MF_plot_table)), E05_MF_plot_table)
R2_MF_plot_table <- cbind("Genotype" = rep('R2', nrow(R2_MF_plot_table)), R2_MF_plot_table)
PA_MF_plot_table <- cbind("Genotype" = rep('PA', nrow(PA_MF_plot_table)), PA_MF_plot_table)
Sierra_MF_plot_table <- cbind("Genotype" = rep('Sierra', nrow(Sierra_MF_plot_table)), Sierra_MF_plot_table)
...
all_genotype_MF_plot_table <- rbind(Y05_MF_plot_table, Y023_MF_plot_table, E05_MF_plot_table, R2_MF_plot_table, PA_MF_plot_table, 
    ...
    #Make plot
x_axis_MF_order <- factor(all_genotype_MF_plot_table$Genotype, levels = c('Y05', 'Y023', 'E05', 'R2', 'PA', 'Sierra'))

p_MF <- ggplot(data = all_genotype_MF_plot_table, aes(x = x_axis_MF_order, y = Term, size = Significant, color = p_adjusted))
p_MF <- p_MF + geom_point() + scale_color_gradientn(colors = terrain.colors(10), limits=c(0, 0.05)) 
p_MF <- p_MF + xlab('Genotype') + ylab('GO Term') + ggtitle('MF') +theme(plot.title = element_text(hjust = 0.5))
p_MF <- p_MF + labs(color = 'Adjusted p-value', size = 'Gene Rank')
...
Sierra_CC_plot_table <- Sierra_CC_GO_terms[which(Sierra_CC_GO_terms$GO.ID %in% list_all_CC_GO_included), ]
    ...
    #Add a column labeling the genotype to each 
Y05_CC_plot_table <- cbind("Genotype" = rep('Y05', nrow(Y05_CC_plot_table)), Y05_CC_plot_table)
Y023_CC_plot_table <- cbind("Genotype" = rep('Y023', nrow(Y023_CC_plot_table)), Y023_CC_plot_table)
E05_CC_plot_table <- cbind("Genotype" = rep('E05', nrow(E05_CC_plot_table)), E05_CC_plot_table)
R2_CC_plot_table <- cbind("Genotype" = rep('R2', nrow(R2_CC_plot_table)), R2_CC_plot_table)
PA_CC_plot_table <- cbind("Genotype" = rep('PA', nrow(PA_CC_plot_table)), PA_CC_plot_table)
Sierra_CC_plot_table <- cbind("Genotype" = rep('Sierra', nrow(Sierra_CC_plot_table)), Sierra_CC_plot_table)
...
all_genotype_CC_plot_table <- rbind(Y05_CC_plot_table, Y023_CC_plot_table, E05_CC_plot_table, R2_CC_plot_table, PA_CC_plot_table, 
    ...
    #Make plot
x_axis_CC_order <- factor(all_genotype_CC_plot_table$Genotype, levels = c('Y05', 'Y023', 'E05', 'R2', 'PA', 'Sierra'))

p_CC <- ggplot(data = all_genotype_CC_plot_table, aes(x = x_axis_CC_order, y = Term, size = Significant, color = p_adjusted))
p_CC <- p_CC + geom_point() + scale_color_gradientn(colors = terrain.colors(10), limits=c(0, 0.05)) 
p_CC <- p_CC + xlab('Genotype') + ylab('GO Term') + ggtitle('CC') +theme(plot.title = element_text(hjust = 0.5))
p_CC <- p_CC + labs(color = 'Adjusted p-value', size = 'Gene Rank')

#--------------------------------------------------------------------------------------------------

#Display plots in 1x3 grid... NEEDS WORK
#final_plot <- grid.arrange(p_BP, p_MF, p_CC, ncol = 1)
#ggsave('GOenrich_Dotplot_CustomAnnotation_AllGenes.pdf', plot = final_plot, device = 'pdf')


#only plot significant, facet grid, change color and/or background, split script, R package WGCNAR (weighted gene coexpression analysis)
